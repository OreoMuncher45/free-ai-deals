import { promos, providers, SITE_URL } from "@/data/providers";
...
export default function PromosIndex() {
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 md:py-10">
      ...
      <div className="mt-8 grid gap-px overflow-hidden rounded-2xl border border-[#1c2534] bg-[#1c2534] sm:grid-cols-2">
        {promos.map((pr, i) => {
          ...
          return (
            <article key={pr.slug} className="bg-[#0c111b] p-5">
              ...
              <div className="mt-3 flex flex-col gap-2 font-mono text-[12px] sm:flex-row">
                {p && <a href={`/providers/${p.slug}`} className="border border-[#1c2534] rounded-full px-3 py-3 text-center font-bold text-white hover:border-[#ff4d00]">More details →</a>}
                <a href="/promos/1-dollar-deals" className="bg-[#ff4d00] rounded-full px-3 py-3 text-center font-bold text-white hover:bg-white hover:text-black">$1 deals</a>
              </div>
            </article>
          );
        })}
