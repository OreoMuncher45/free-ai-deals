      ...
      <table className="w-full min-w-[640px] border-collapse bg-[#0c111b] font-mono text-[12px]">
        <thead>
          <tr className="bg-black text-left text-white">
            <th className="border border-[#1c2534] px-3 py-2.5">Route</th>
            <th className="border border-[#1c2534] px-3 py-2.5">What you get</th>
            <th className="border border-[#1c2534] px-3 py-2.5">Cost</th>
            <th className="border border-[#1c2534] px-3 py-2.5">Verify</th>
          </tr>
        </thead>
        <tbody>
          {routes.map((r) => (
            <tr key={r.provider + r.what} className="hover:bg-[#101724]">
              ...
              <td className="border border-[#1c2534] px-3 py-2.5 text-[#e8eef6]">{r.what}</td>
              <td className="border border-[#1c2534] px-3 py-2.5 text-[#ff4d00]">{r.cost}</td>
              <td className="border border-[#1c2534] px-3 py-2.5 text-[#8b98ad]">{r.verify}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 md:py-10">
      ...
      <nav aria-label="Back" className="flex flex-wrap gap-2 font-mono text-[12px]">
        ...
        <a href="/models" className="border border-[#1c2534] bg-[#0c111b] px-3 py-2.5 text-white hover:border-[#ff4d00]">← All models</a>
      </nav>
      ...
      <div className="mt-4 grid gap-px border border-[#1c2534] bg-[#1c2534] sm:grid-cols-2">
        {related.map((r) => (
      ...
      <div className="mt-10 flex flex-col gap-2 border-t border-[#1c2534] pt-6 font-mono text-[12px] sm:flex-row">
        ...
        <a href="/models" className="border border-[#1c2534] px-4 py-3.5 text-center text-white hover:border-[#ff4d00]">← All models</a>
      </div>
    </main>
  );
}
